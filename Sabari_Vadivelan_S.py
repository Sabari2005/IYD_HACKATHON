...
def main(question_csv_file_path, output_file_path): 
    ...
    # faiss_db_url = "https://drive.google.com/drive/folders/1Qrhpbh5MWuIReg33sERqsMF8tJPKe1FM?usp=sharing"
    folder_id = "1Qrhpbh5MWuIReg33sERqsMF8tJPKe1FM"
    gdown.download_folder(id=folder_id, output="faiss_db1", quiet=False, use_cookies=False)

    #Ramayana_faiss_db_url = "https://drive.google.com/drive/folders/1Y5Hw0NWXP4imiTcH7PAYIqMwzq42ghaV?usp=sharing"
    folder_id = "1Y5Hw0NWXP4imiTcH7PAYIqMwzq42ghaV"
    gdown.download_folder(id=folder_id, output="ramayana_faiss_db", quiet=False, use_cookies=False)

    # Download the CSV
    csv_file_id = "1wJWSfPWqTVCnE4XHr4iDRBOvBTXDbNBE"
    csv_output = "valmiki.csv"
    gdown.download(id=csv_file_id, output=csv_output, quiet=False)

    # Download PDF
    pdf_file_id = "1k-QiQjhOsg_LunhW9iCv1mj0O9GXVD8h"
    pdf_output = "Books.pdf"
    gdown.download(id=pdf_file_id, output=pdf_output, quiet=False)

    # Download the .env file
    env_file_id = "1eBBQckt8LCoVB0gKRamJvJkYtSHCz2NL"
    env_output = ".env"
    gdown.download(id=env_file_id, output=env_output, quiet=False)
...
question_csv_file_path="test.csv" #example : test.csv
output_file_path="result.csv" #examplt : result.csv
